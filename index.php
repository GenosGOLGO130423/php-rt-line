...
if (!empty($_POST)) {
	if($_POST['message'] !=='') {
			...
			$message->execute(array(
				$member['id'],
				$_POST['message'],
				$_POST['reply_post_id'] //input name="reply_post_id"
			));
...
$page = max($page,1);//マイナスの数字を入れられても1が返される
//100のような大きな数字を入れられた場合はどうするか？まず最終Pを取得する。
...
$posts->execute();
//返信の場合
if (isset($_REQUEST['res'])) { //[RE]がクリックされたとき
  ...
  <div id="content">
		...
		<div class="msg">
			<img src="member_picture/<?php print htmlspecialchars($post['picture'],ENT_QUOTES); ?>" width="48" height="48" alt="<?php print htmlspecialchars($post['name'],ENT_QUOTES); ?>" />
				...
				[<a href="delete.php?id=<?php print htmlspecialchars($post['id']); ?>"　style="color: #ff3333;">削除</a>]
			<?php endif; ?>
			</p>
		</div>
